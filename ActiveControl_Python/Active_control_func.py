...
#%%
def get_accelerations(B_sec,B_wfbk):
    ...
    return th,xh,errwocompression,err
...
for i in B_secs:
   for j in B_wfbks:
    th,xh, err_wo_compression,err =    get_accelerations(i,j)
    err_wo_compressions.append(err_wo_compression)
    errs.append(err)
...
fig = make_subplots(
    rows=3, 
    cols=3,
    subplot_titles=[f"Branches: wsec {(i)%3 + 1} wfbk {1 + int((i)/3)}" for i in range(9)],
    vertical_spacing=0.15,
    horizontal_spacing=0.08
)

# Assuming th, xh, errwocompression, err are your data arrays
# Add the same traces to each subplot
for i in range(1, 10):  # 9 subplots (3x3)
    row = (i-1)//3 + 1
    col = (i-1)%3 + 1
    
    # Add Perturbation force
    fig.add_trace(
        go.Scatter(x=th, y=xh, name="Perturbation force (N)", mode="lines", 
    ...
    # Add Accel. w.o. compr.
    fig.add_trace(
        go.Scatter(x=th, y=err_wo_compressions[i-1], name="Accel. w.o. compr. (m/s²)", mode="lines",
                  line=dict(color='red'), showlegend=(i==1)),
        row=row, col=col
    )
    
    # Add Beam acceleration
    fig.add_trace(
        go.Scatter(x=th, y=errs[i-1], name="Beam acceleration (m/s²)", mode="lines",
...
fig.update_layout(
    height=800,
    width=1000,
    title_text="3x3 Grid of Acceleration and Force Plots",
    title_x=0.5
)

# Update axes labels for all subplots
for i in range(1, 10):
    fig.update_xaxes(title_text="Time", row=(i-1)//3 + 1, col=(i-1)%3 + 1)
    fig.update_yaxes(title_text="Value", row=(i-1)//3 + 1, col=(i-1)%3 + 1)

fig.show()
# %%
fig = px.line()

fig.add_scatter(x = th, y=errs[5])
fig.add_scatter(x = th, y=errs[8])
# %%
